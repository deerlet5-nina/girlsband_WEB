...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const entrezConfig = {
        ...
        tool: 'genshin_website',
        email: '2278096631@qq.com'
    };
    ...
    function displayResults(docSums) {
        ...
        resultsHeader.textContent = `共找到 ${currentSearch.totalResults} 条结果，显示第 ${currentSearch.currentPage + 1} 页`;
    ...
    function updatePagination() {
        ...
        // 调整起始页，确保显示足够的按钮
        if (endPage - startPage + 1 < maxButtons && startPage > 0) {
    ...
    function goToPage(pageNumber) {
        ...
        // 构建API请求URL，包含分页参数
        let searchUrl = `${entrezConfig.baseUrl}esearch.fcgi?db=${currentSearch.db}&term=${encodeURIComponent(currentSearch.term)}&retmode=xml&retstart=${pageNumber * currentSearch.resultsPerPage}&retmax=${currentSearch.resultsPerPage}&tool=${entrezConfig.tool}&email=${entrezConfig.email}`;
